...
async function test() {
  ...
  if (res1.status === 303 || res1.status === 302) {
    ...
    const req = https.get(loc, { headers: { Cookie: cookieStr, Range: "bytes=50000000-50000100" } }, (proxyRes) => {
      console.log("https.get statusCode:", proxyRes.statusCode);
      console.log("https.get headers content-range:", proxyRes.headers['content-range']);
      proxyRes.destroy();
    }).on('error', (err) => {
